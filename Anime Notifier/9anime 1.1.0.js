...
// --------------Main Code--------------
const animeUrl = "https://www13.9anime.to/home"
fileCheck()
let latestAnimes = await getLatestAnime()
let notifyAnimes = animeNotifs
let notifyCheck = latestAnimeCheck(latestAnimes)
...
async function getLatestAnime() {
  ...
  for (let animeRaw of animesRaw) {
    ...
    let imgUrl = animeRaw.slice(imgUrlStart+5, imgUrlEnd)
...
function latestAnimeCheck(newAnimes) {
  ...
  let animeFromJSON = JSON.parse(FileManager.iCloud().readString(currentAnimeJSON))
  
  let notif = null
  if (animeFromJSON.title != currentAnime.title) {
    let userAnimesLow = userAnimesLower(notifyAnimes)
    let titleLow = (newAnimes[0].title).toLowerCase()
    
    if (userAnimesLow.indexOf(titleLow) > -1) {
      notif = createNotification(latestAnimes)
    }
    
    FileManager.iCloud()
      .writeString(currentAnimeJSON, JSON.stringify(currentAnime))
...
function fileCheck() {  
  ...
  let filePath = fm.joinPath(dirPath,
    "/animeWidget/currentAnime.json")
